...
public class AnalyzerProfiler extends Analyzer {
    ...
    public AnalyzerProfiler(Logger logger) {
        super(logger); metrics.Profiler.write("entry;metrics.AnalyzerProfiler;public AnalyzerProfiler(Logger logger)"); 
     metrics.Profiler.write("exit;metrics.AnalyzerProfiler;public AnalyzerProfiler(Logger logger)"); }
    @Override
    public String analyzeFile(File file, Object additionalParam) { metrics.Profiler.write("entry;metrics.AnalyzerProfiler;public String analyzeFile(File file, Object additionalParam)"); 
        String contents = Tools.readFile(file);
        contents = contents.replaceAll("\t", "    ");
        
        //                  0            1        2          3     4    
        String[] tokens = {"package ", "return ", "class ",  "{", "}"};
...
//            logger.log("token: " + tokens[result[1]], 3);
            ...
            if (result[1] == 0) { //package
                ...
                if ((packageName + className).equals("metrics.Profiler")) {
...
//                    command = " try{Thread.sleep(3000);}catch(Exception e){} ";
        ...
        try {
            ...
            logger.log("Error occurred when writing file", e, 1);
             metrics.Profiler.write("exit;metrics.AnalyzerProfiler;public String analyzeFile(File file, Object additionalParam)"); return null;
        }
